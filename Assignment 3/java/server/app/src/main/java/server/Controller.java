...
public class Controller {
    ...
    // Example functions
    // The following are examples of what you can do with Spring, you will probably want remove them on submission

    /*
     This function runs when the url http://<host>:<port>/ is requested,
     sub-urls can be specified but putting different arguments into @GetMapping(args)
    ...
    The contents of the curly brackets in the route are the name of variable which gains the value
    of what the client enters in its place. E.g. if the client requests `http://127.0.0.1:2250/hello/charlie`
    then name = "charlie".
     */
    ...
    to the function.
    If json data is sent, you can instead use a Class with corresponding variables as the argument.
    */
    @PostMapping("/hello")
    ...
    /* With this, we show how to redirect a client to a different function that we provide. */
    @GetMapping("/redirect")
    ...
    */
    ...
    it gets translated into a json file and sent back to the client. This can be a bit limited, so
    ...
    */
    ...
    public Map<String, Object> auditExpenses() {
        try (
    ...
    public String addExpense(@RequestBody Map<String, String> newExpense) {
        try (
            BufferedWriter fileOut = new BufferedWriter(new FileWriter("expenses.txt", true))
        ) {
            fileOut.append(newExpense.get("value") + "\n");
